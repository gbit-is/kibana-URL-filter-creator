...
port = 8098 # Port for flask server



base = "https://your_kibana_url/app/kibana#/dashboard/" # Base URL, check http/https in the url
defaultBoardId = "75554d5f-6e8c-42a9-900c-958553423428" # If no ID is provided, this gets used (UUID provided is random)
...
def makeParam(field,match): # field is the field and match is, what we wanna match, this only supports "field is something" filters 
    base = '''('$state':(store:appState),meta:(disabled:!f,key:FIELD,negate:!f,params:(query:MATCH),type:phrase,value:MATCH),query:(match:(FIELD:(query:MATCH,type:phrase))))''' # Base string 
...
def makeUrl(id,params): 
    ...
    if uuid == "failed":
        ...
        for param in params:
            res = makeParam(param[0],param[1]) # send the parameter provided to the makeParam function
            url = url + res
...
def hello():
    ...
    url = makeUrl(id,params) # Send all params to the URL creation function
...
def invalid():
    ...
    message = "<br><br><h2>You provided an invalid ID that is neither a UUID nor a defined alias<br>Id that was provided was:<b> " + str(id) + "</b></h2>"
    return message
